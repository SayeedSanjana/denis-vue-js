      ...
      <div class="flex items-center justify-center p-6 mx-3 my-4  ">
        ...
        <form @submit.prevent="updatePatientInfo(this.$route.params.id)" class="w-full max-w-lg ">
          ...
          <div class="w-full lg:w-full ">
            ...
            <input @blur="v$.formData.nid.$touch()"  class="appearance-none block w-full bg-white text-regal-teal border  border-regal-teal border-opacity-50 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:border-regal-blue" id="nid" type="text" placeholder="Enter NID" v-model="formData.nid">
  ...
  export default {
    ...
    data() {
     ...
     validations(){
        ...
        nid: {minLength: minLength(9),maxLength: maxLength(20)}
			}
    ...
    methods: {
      ...
      async updatePatientInfo(id) {
        ...
        try {
          ...
          // await axios.put(`patients/${id}`, this.formData, {
          //     headers: {
          //       "Authorization": `Bearer ${localStorage.getItem('token') }`
          //     }
          //   })
